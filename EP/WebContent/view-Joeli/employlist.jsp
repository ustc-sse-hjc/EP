	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},   
 		        ...
 		        {field:'pos',title:'职位',width:150},
 		       {field:'id',title:'身份证',width:200, sortable: true}, 
 		        {field:'dept_id',title:'部门',width:150, 
 		        	formatter: function(value,row,index){
 						if (row.deptId){
 							var deptList = $("#deptList").combobox("getData");
 							for(var i=0;i<deptList.length;i++ ){
 								//console.log(deptList[i]);
 								if(row.deptId == deptList[i].id)return deptList[i].name;
 							}
 							return row.deptId;
 						} else {
 							return 'not found';
	    ...
	    $("#delete").click(function(){
	  	...
	  	function preLoaddept(){
	  		$("#deptList").combobox({
		  		...
		  		editable: false, //不可编辑
		  		method: "post",
		  		url: "deptServlet?method=getdeptList&t="+new Date().getTime()+"&from=combox",
